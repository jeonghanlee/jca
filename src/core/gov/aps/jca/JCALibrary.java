 ...
 *
 *
 * $Id: JCALibrary.java,v 1.3 2004/03/31 09:14:11 boucher Exp $
 *
 */
...
/** The JCALibrary class is the entry point to all JCA enabled application.<br>
...
public final class JCALibrary {
  ...
  static private final int REVISION=1;
  static private final int MODIFICATION=0;
   ...
   */
  static public JCALibrary getInstance() {
    if( _instance==null ) {
  ...
  protected JCALibrary() {
    ...
    // Read Configuration
    try {
      _builtinProperties.load( JCALibrary.class.getResourceAsStream(
      "JCALibrary.properties" ) );
    } catch( Throwable ressourceEx ) {
      System.out.println( "Unable to load ressource\n"+ressourceEx );
    }
  ...
  /** Constant string representing the fully qualified class name of a thread safe Context implementation.<br>
   ...
   *
   *<br>
   * <b>Note:</b> If you use the prefix <b>gov.aps.jca.jni.JNIContext</b> instead of <b>gov.aps.jca.jni.ThreadSafeContext</b>
   * then the property will affect both JNI_THREAD_SAFE and JNI_SINGLE_THREADED context configuration.<br><br>
   *
   *
   *<table width="100%" border="0" cellspacing="0" cellpadding="0">
   * <tr>
  ...
  public Context createContext(Configuration configuration) throws CAException {
    try {
      ...
      throw new CAException("Unable to create context", ex);
    }
  }
