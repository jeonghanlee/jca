 ...
 *
 *
 * $Id: SingleThreadedContext.java,v 1.3 2004/03/31 09:14:11 boucher Exp $
 *
 * Modification Log:
...
final public class SingleThreadedContext extends JNIContext implements Configurable {
  ...
  public Channel createChannel( String name, ConnectionListener l ) throws CAException, IllegalStateException {
    ...
    assertThread();
      JNIChannel channel=new JNIChannel( this, name, l );
      ...
      synchronized( JNI.lock() ) {
        ...
        try {
          channel.setChannelID( JNI.ch_channelCreate( name, callback ) );
        } catch(JNIException ex) {
  ...
  void ch_arrayPut( DBRType type, int count, JNIChannel ch, Object value ) throws JNIException {
    ...
    assertThread();
      ...
      int dbrid=JNI.dbr_create( type.getValue(), count );
