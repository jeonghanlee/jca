 ...
 *
 *
 * $Id: JNIContext.java,v 1.4 2004/04/01 09:52:58 boucher Exp $
 *
 * Modification Log:
...
abstract public class JNIContext extends Context {
  ...
  protected JNIContext() {
    ...
    try {
      setEventDispatcher( ( EventDispatcher )Class.forName( jca.getProperty(
        cn+".event_dispatcher", "" ) ).newInstance());
...
//      if(getLogger()!=null) getLogger().log(Level.SEVERE, "Unable to initialize context", ex);
...
//    assertState(isValid(),  "Context has been destroyed");
   ...
   */
  abstract protected int contextInitialize() throws CAException;
  ...
  void registerChannel(JNIChannel ch) {
    if(ch==null) return;
    _channelMap.put(new Integer(ch.getChannelID()), new WeakReference(ch));
  }

  JNIChannel lookupChannel(int channelID) {
    WeakReference ref= (WeakReference) _channelMap.get(new Integer(channelID));
  ...
  void unregisterChannel(JNIChannel ch) {
    if(ch==null) return;
    _channelMap.remove(new Integer(ch.getChannelID()));
  }
